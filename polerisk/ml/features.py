    ...
    def fit(self, df: pd.DataFrame) -> "FeatureEngineer":
        ...
        for col in ["in_situ", "satellite"]:
            if col in df.columns:
                self.seasonal_stats[f"{col}_monthly_mean"] = df.groupby("month")[
                    col
                ].mean()
                self.seasonal_stats[f"{col}_monthly_std"] = df.groupby("month")[
                    col
                ].std()
        ...
        for col in ["satellite", "in_situ"]:
            if col in features.columns:
                ...
                for window in [3, 7, 14, 30]:
                    features[f"{col}_ma_{window}"] = (
                        features[col].rolling(window=window, center=True).mean()
                    )

                # Moving standard deviations
                for window in [7, 14]:
                    features[f"{col}_std_{window}"] = (
                        features[col].rolling(window=window, center=True).std()
                    )
                ...
                features[f"{col}_roc_3"] = (
                    features[col] / features[col].shift(3) - 1
                ) * 100
                features[f"{col}_roc_7"] = (
                    features[col] / features[col].shift(7) - 1
                ) * 100
        ...
        if "date" in features.columns:
            ...
            for col in ["in_situ", "satellite"]:
                if (
                    ...
                    and f"{col}_monthly_mean" in self.seasonal_stats
                ):
                    monthly_mean = features["month"].map(
                        self.seasonal_stats[f"{col}_monthly_mean"]
                    )
                    monthly_std = features["month"].map(
                        self.seasonal_stats[f"{col}_monthly_std"]
                    )

                    features[f"{col}_seasonal_anomaly"] = (
                        features[col] - monthly_mean
                    ) / monthly_std
...
def create_weather_features(
    ...
    if precipitation_col and precipitation_col in features.columns:
        ...
        features["days_since_rain"] = (
            rain_days[::-1].groupby(rain_days[::-1].cumsum()).cumcount()[::-1]
        )
...
def create_satellite_quality_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    features["satellite_change_abs_ma3"] = (
        features["satellite_change_abs"].rolling(3).mean()
    )
...
def select_features(
    ...
    correlations = (
        df[numeric_cols].corrwith(df[target_col]).abs().sort_values(ascending=False)
    )
    ...
    logger.info(
        f"Selected {len(selected_features)} features out of {len(numeric_cols)} total"
    )
