...
class PoleFailurePredictionModel:
    ...
    def _initialize_model(self):
        ...
        if self.model_type == "random_forest":
            ...
            if not TENSORFLOW_AVAILABLE:
                logger.warning(
                    "TensorFlow not available, falling back to Random Forest"
                )
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "pole_type" in features.columns:
            ...
            pole_type_dummies = pd.get_dummies(
                features["pole_type"], prefix="pole_type"
            )
        ...
        if "overall_health_score" in features.columns:
            features["health_critical"] = (
                features["overall_health_score"] < 30
            ).astype(int)
            features["health_poor"] = (features["overall_health_score"] < 50).astype(
                int
            )
            features["health_fair"] = (features["overall_health_score"] < 70).astype(
                int
            )
        ...
        if available_risk_cols:
            ...
            features["risk_factor_count"] = (features[available_risk_cols] > 0.5).sum(
                axis=1
            )

        # Environmental factors
        if "voltage_class" in features.columns:
            voltage_dummies = pd.get_dummies(
                features["voltage_class"], prefix="voltage"
            )
        ...
        if "maintenance_priority" in features.columns:
            priority_dummies = pd.get_dummies(
                features["maintenance_priority"], prefix="priority"
            )
            features = pd.concat([features, priority_dummies], axis=1)

        # Interaction features
        if (
            "age_years" in features.columns
            and "overall_health_score" in features.columns
        ):
            features["age_health_interaction"] = features["age_years"] * (
                100 - features["overall_health_score"]
            )

        return features

    def create_failure_labels(
        self, df: pd.DataFrame, failure_threshold: float = 30
    ) -> pd.Series:
        """
        ...
        if "maintenance_priority" in df.columns:
            failure_conditions.append(
                df["maintenance_priority"].isin(["critical", "high"])
            )
    ...
    def fit(
        ...
        metrics = {
            ...
            "val_auc": (
                roc_auc_score(y_val, y_pred_proba) if len(np.unique(y_val)) > 1 else 0.5
            ),
        ...
        self.is_fitted = True
        logger.info(
            f"Failure prediction model trained. Validation F1: {metrics['val_f1']:.4f}"
        )
    ...
    def predict_failure_probability(
        ...
        if isinstance(X, pd.DataFrame):
            ...
            if self.feature_names:
                ...
                if missing_features:
                    logger.warning(
                        f"Missing features in prediction data: {missing_features}"
                    )
...
class AnomalyDetector:
    ...
    def detect_anomalies(
        ...
        if self.method == "isolation_forest":
            anomaly_labels = self.model.predict(
                X_scaled
            ...
            z_scores = np.abs(
                (X_scaled - self.threshold["mean"]) / self.threshold["std"]
            )
...
class TimeSeriesForecaster:
    ...
    def fit(self, time_series: Union[pd.Series, np.ndarray]) -> Dict:
        ...
        if self.model_type == "lstm":
            ...
            self.model = keras.Sequential(
                [
                    layers.LSTM(
                        50, return_sequences=True, input_shape=(self.sequence_length, 1)
                    ),
                    layers.Dropout(0.2),
                    layers.LSTM(50),
                    layers.Dropout(0.2),
    ...
    def forecast(
        self, time_series: Union[pd.Series, np.ndarray], steps: Optional[int] = None
    ) -> np.ndarray:
        """
        Generate forecasts.
        ...
        if self.model_type == "lstm":
            ...
            X_future = np.arange(len(scaled_data), len(scaled_data) + steps).reshape(
                -1, 1
            )
            forecasts = self.model.predict(X_future)

        # Inverse transform
        forecasts_rescaled = self.scaler.inverse_transform(
