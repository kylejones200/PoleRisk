...
def train_predictor_command(args):
    ...
    try:
        ...
        if "cv_rmse_mean" in metrics:
            logger.info(
                f"CV RMSE: {metrics['cv_rmse_mean']:.4f} ± {metrics['cv_rmse_std']:.4f}"
            )
        ...
        if importance:
            ...
            sns.barplot(
                x=list(features_to_plot.values()), y=list(features_to_plot.keys())
            )
            ...
            importance_plot = (
                args.output_dir / f"feature_importance_{args.model_type}.png"
            )
...
def detect_anomalies_command(args):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(
        results["date"], results["satellite"], "b-", alpha=0.7, label="Satellite Data"
    )
    ...
    plt.axvline(
        anomaly_scores[anomaly_labels].min(),
        color="red",
        linestyle="--",
...
def forecast_command(args):
    ...
    if len(time_series) < args.min_history:
        raise ValueError(
            f"Not enough historical data. Need at least {args.min_history} points."
        )
    ...
    forecaster = TimeSeriesForecaster(
        model_type=args.model_type, forecast_horizon=args.forecast_days
    )
    ...
    try:
        ...
        forecast_df = pd.DataFrame(
            {"date": forecast_dates, "forecasted_soil_moisture": forecasts}
        )
        ...
        if "date" in df.columns:
            ...
            plt.plot(
                ...
                "r--",
                marker="o",
                label=f"Forecast ({args.forecast_days} days)",
                linewidth=2,
            )
        else:
            plt.plot(
            ...
            plt.plot(
                range(len(recent_data), len(recent_data) + args.forecast_days),
                forecasts,
                "r--",
                marker="o",
                label=f"Forecast ({args.forecast_days} days)",
...
def main():
    ...
    def add_common_args(subparser):
        subparser.add_argument(
            "--data-file", "-d", required=True, help="Path to input data file"
        )
        subparser.add_argument(
            "--output-dir",
            "-o",
    ...
    train_parser.add_argument(
        "--max-features", type=int, default=25, help="Maximum number of features to use"
    )
    train_parser.add_argument(
    ...
    predict_parser.add_argument(
        "--model-file", "-mf", required=True, help="Path to trained model file"
    )
    ...
    anomaly_parser.add_argument(
        "--method",
        choices=["isolation_forest", "statistical"],
    ...
    forecast_parser.add_argument(
        "--forecast-days", type=int, default=7, help="Number of days to forecast"
    )
    forecast_parser.add_argument(
        "--min-history",
        type=int,
