...
class ConfigManager:
    ...
    def get_project_paths(cls) -> Dict[str, Path]:
        ...
        )  # soilmoisture/common/config.py -> project root
    ...
    def get_parameters(cls, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if len(netcdf_files) == 0:
            ...
            lat_lprm, lon_lprm = cls._extract_netcdf_coordinates(
                netcdf_files, config["input_dir"]
            )
        ...
        if standard_insitu_file.exists():
            ...
            if stm_files:
                ...
                config["in_situ"] = (
                    standard_insitu_file  # Keep expected path for error messages
                )
            ...
            try:
                with Dataset(nc_file, "r") as ds:
                    ...
                    if lat_data is not None and lon_data is not None:
                        logger.info(
                            f"Successfully extracted coordinates from {nc_file}"
                        )
