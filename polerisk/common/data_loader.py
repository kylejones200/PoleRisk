...
class DataLoader:
    ...
    def load_csv_data(
        ...
        try:
            ...
            if delimiter == "whitespace":
                df = pd.read_csv(
                    filepath, sep=r"\s+", na_values=na_values, skipinitialspace=True
                )
            else:
                df = pd.read_csv(
                    filepath,
    ...
    def load_pole_data(cls, filepath: Union[str, Path]) -> List[PoleInfo]:
        ...
        logger.info(
            f"Successfully created {len(poles)} PoleInfo objects from {len(df)} rows"
        )
        return poles
    ...
    def load_soil_data(cls, filepath: Union[str, Path]) -> List[SoilSample]:
        ...
        logger.info(
            f"Successfully created {len(samples)} SoilSample objects from {len(df)} rows"
        )
        return samples
    ...
    def load_insitu_data(cls, filepath: Union[str, Path]) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded {len(df)} in-situ measurements from {filepath}"
            )
            return df
    ...
    def _detect_delimiter(filepath: Path) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Could not auto-detect delimiter for {filepath}, using comma"
            )
            return ","
    ...
    def _validate_data_type(df: pd.DataFrame, data_type: str, filepath: Path) -> None:
        ...
        if data_type == "poles":
            ...
            if "moisture_content" in df.columns:
                invalid_moisture = (df["moisture_content"] < 0) | (
                    df["moisture_content"] > 1
                )
                if invalid_moisture.any():
                    logger.warning(
                        f"Suspicious moisture content values in {filepath} (should be 0-1)"
                    )
            ...
            if "ph" in df.columns:
                ...
                if invalid_ph.any():
                    logger.warning(
                        f"Suspicious pH values in {filepath} (should be 0-14)"
                    )
