...
class SurvivalAnalysisModel:
    ...
    def estimate_remaining_useful_life(
        ...
        conditional_survival_future = conditional_survival[
            survival_curve.ages > current_age
        ]
        ...
        if len(conditional_survival_future) > 0:
            ...
            median_rul = np.interp(
                0.5, conditional_survival_future[::-1], future_ages[::-1]
            )

            # Confidence band (5th and 95th percentiles)
            lower_bound = np.interp(
                0.95, conditional_survival_future[::-1], future_ages[::-1]
            )
            upper_bound = np.interp(
                0.05, conditional_survival_future[::-1], future_ages[::-1]
            )
        else:
            expected_rul = 0.0
            median_rul = 0.0
        ...
        for horizon in horizons:
            ...
            if future_age <= survival_curve.ages[-1]:
                survival_prob = np.interp(
                    future_age, survival_curve.ages, survival_probs
                )
                conditional_prob = (
                    survival_prob / current_survival_prob
                    if current_survival_prob > 0
                    else 0.0
                )
    ...
    def batch_estimate_rul(
        ...
        health_score_col: Optional[str] = None,
    ) -> pd.DataFrame:
        """
        ...
        for _, row in poles_df.iterrows():
            try:
                ...
                health_score = (
                    row[health_score_col]
                    if health_score_col and health_score_col in row
                    else None
                )
                ...
                logger.warning(
                    f"Error estimating RUL for pole {row.get('pole_id', 'unknown')}: {e}"
                )
                continue
    ...
    def _weibull_survival(
        self, t: np.ndarray, shape: float, scale: float
    ) -> np.ndarray:
    ...
    def _initialize_baseline_params(
        self,
    ) -> Dict[Tuple[PoleClass, EnvironmentType], Tuple[float, float]]:
