...
class InspectionRealismModel:
    ...
    def simulate_inspection(
        ...
        for defect in actual_defects:
            ...
            for method in methods:
                ...
                consistency_factor = np.random.normal(
                    inspector.consistency, 0.1 * (1 - inspector.consistency)
                )
            ...
            defect_copy = Defect(
                ...
                * inspector.severity_bias,  # Apply severity bias
        ...
        detection_rate = len([d for d in detected_defects if d.actual_present]) / max(
            len(actual_defects), 1
        )
    ...
    def calculate_optimal_inspection_interval(
        self, pole_risk_score: float, defect_growth_rate: float = 0.1
    ) -> int:
        """
    ...
    def recommend_inspection_methods(
        self, defect_types: List[DefectType], budget_per_pole: float = 500.0
        ...
        remaining_budget = (
            budget_per_pole
            - self.method_capabilities[InspectionMethod.VISUAL].cost_per_pole
        )
        ...
        for method, score_per_dollar in sorted(
    ...
    def _get_detection_probability(
        self, defect: Defect, method: InspectionMethod
    ) -> float:
    ...
    def _get_skill_modifier(
        self, inspector: InspectorProfile, method: InspectionMethod
    ) -> float:
        """Get skill modifier based on inspector capabilities."""
        if method == InspectionMethod.VISUAL:
            return inspector.visual_skill
        elif method in [
            InspectionMethod.RESISTOGRAPH,
    ...
    def _calculate_environmental_modifier(
        self, method: InspectionMethod, conditions: Dict
    ) -> float:
    ...
    def _calculate_inspection_confidence(
        self,
        methods: List[InspectionMethod],
        inspector: InspectorProfile,
        conditions: Dict,
        ...
        method_confidence = np.mean(
            [self.method_capabilities[m].detection_probability for m in methods]
        )
        ...
        overall_confidence = (method_confidence * 0.5 + inspector_confidence * 0.5) * (
            1.0 - env_penalty
        )

        return np.clip(overall_confidence, 0.0, 1.0)

    def _initialize_detection_matrix(
        self,
    ) -> Dict[Tuple[DefectType, InspectionMethod], float]:
        ...
        matrix[
            (DefectType.OVERLOAD, InspectionMethod.STRUCTURAL_ENGINEERING_REVIEW)
        ] = 0.95
        matrix[(DefectType.CRACK, InspectionMethod.STRUCTURAL_ENGINEERING_REVIEW)] = (
            0.90
        )
        matrix[
            (
                DefectType.FOUNDATION_ISSUE,
                InspectionMethod.STRUCTURAL_ENGINEERING_REVIEW,
            )
        ] = 0.85

        # Ground penetrating radar
        matrix[
            (DefectType.FOUNDATION_ISSUE, InspectionMethod.GROUND_PENETRATING_RADAR)
        ] = 0.85

        return matrix

    def _initialize_method_capabilities(
        self,
    ) -> Dict[InspectionMethod, InspectionCapability]:
        ...
        capabilities[InspectionMethod.STRUCTURAL_ENGINEERING_REVIEW] = (
