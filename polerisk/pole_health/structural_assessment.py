...
class StructuralConditionAnalyzer:
    ...
    def assess_wood_condition(
        ...
        if inspection.wood_decay_depth is not None:
            decay_risk = 0.0
            if (
                inspection.wood_decay_depth
                >= self.wood_thresholds["decay_depth_critical"]
            ):
                decay_risk = 1.0
            elif (
                inspection.wood_decay_depth >= self.wood_thresholds["decay_depth_poor"]
            ):
                decay_risk = 0.8
            elif (
        ...
        if inspection.wood_circumferential_loss is not None:
            ...
            if loss >= self.wood_thresholds["circumferential_loss_critical"]:
                ...
                circ_risk = (
                    loss / self.wood_thresholds["circumferential_loss_fair"] * 0.3
                )
    ...
    def assess_concrete_condition(
        ...
        if inspection.concrete_cracking is not None:
            risks["concrete_cracking_risk"] = (
                0.6 if inspection.concrete_cracking else 0.0
            )

        # Spalling assessment
        if inspection.concrete_spalling is not None:
            risks["concrete_spalling_risk"] = (
                0.7 if inspection.concrete_spalling else 0.0
            )

        return risks

    def assess_steel_condition(
        self, inspection: StructuralInspection
    ) -> Dict[str, float]:
        ...
        if inspection.steel_corrosion_level is not None:
            ...
            if corr_level >= self.steel_thresholds["corrosion_level_critical"]:
                ...
                corr_risk = (
                    corr_level / self.steel_thresholds["corrosion_level_poor"] * 0.5
                )
    ...
    def assess_structural_geometry(
        self, inspection: StructuralInspection
    ) -> Dict[str, float]:
        ...
        if inspection.twist_angle is not None:
            ...
            if twist >= self.structural_limits["twist_angle_critical"]:
                ...
                twist_risk = (
                    twist / self.structural_limits["twist_angle_critical"] * 0.8
                )
    ...
    def assess_remaining_strength(
        ...
        if inspection.estimated_remaining_strength is not None:
            ...
            if strength_loss >= self.structural_limits["strength_loss_critical"]:
                ...
                strength_risk = (
                    strength_loss / self.structural_limits["strength_loss_poor"] * 0.6
                )
    ...
    def calculate_overall_condition_score(
        self, inspection: StructuralInspection, pole_type: str
    ) -> float:
    ...
    def determine_inspection_frequency(
        self, condition_score: float, pole_age: float, pole_type: str
    ) -> int:
    ...
    def generate_recommendations(
        ...
        # Geometry issues
        if (
            inspection.lean_angle is not None
            and abs(inspection.lean_angle) > self.structural_limits["lean_angle_poor"]
        ):
...
class EnhancedPoleHealthAssessment:
    ...
    def assess_pole_with_structural_data(
        self,
        pole: PoleInfo,
        soil_samples: List,
        structural_inspections: List[StructuralInspection] = None,
        load_analysis: LoadAnalysis = None,
    ) -> PoleHealthMetrics:
        """
        ...
        if structural_inspections:
            latest_inspection = max(
                structural_inspections, key=lambda x: x.inspection_date
            )

            # Calculate structural condition score
            structural_score = (
                self.structural_analyzer.calculate_overall_condition_score(
            ...
            structural_recommendations = (
                self.structural_analyzer.generate_recommendations(
            ...
            inspection_frequency = (
                self.structural_analyzer.determine_inspection_frequency(
            ...
            elif structural_score < 50 and base_metrics.maintenance_priority in [
                "low",
                "medium",
            ]:
