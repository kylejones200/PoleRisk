...
class PoleRiskScorer:
    ...
    def calculate_comprehensive_risk_score(
        ...
        service_area_data: Optional[Dict] = None,
    ) -> Dict[str, float]:
        """
        ...
        safety_risk = self._calculate_safety_risk(
            pole, health_metrics, service_area_data
        )
    ...
    def _calculate_financial_risk(
        self, pole: PoleInfo, health_metrics: PoleHealthMetrics
    ) -> float:
    ...
    def _calculate_environmental_risk(
        ...
        if pole.pole_type.lower() == "wood" and pole.treatment_type:
            if (
                "CCA" in pole.treatment_type
                or "pentachlorophenol" in pole.treatment_type
            ):
...
class MaintenanceScheduler:
    ...
    def create_maintenance_schedule(
        self,
        poles_data: List[Tuple[PoleInfo, PoleHealthMetrics]],
    ...
    def _determine_maintenance_window(self, metrics: PoleHealthMetrics) -> str:
        ...
        if metrics.requires_immediate_attention or metrics.overall_health_score < 20:
            ...
            metrics.maintenance_priority == "high" or metrics.overall_health_score < 40
        ):
            return "urgent"
        elif (
            metrics.maintenance_priority == "medium"
            or metrics.overall_health_score < 70
        ):
            return "planned"
        else:
            return "routine"
    ...
    def _determine_recommended_actions(
        self, pole: PoleInfo, metrics: PoleHealthMetrics
    ) -> List[str]:
    ...
    def generate_maintenance_report(
        self, schedule_df: pd.DataFrame, budget_limit: Optional[float] = None
    ) -> Dict:
        ...
        urgent_poles = len(
            schedule_df[schedule_df["maintenance_window"].isin(["emergency", "urgent"])]
        )
        ...
        report["summary"] = {
            ...
            "average_cost_per_pole": (
                total_estimated_cost / total_poles if total_poles > 0 else 0
            ),
            "urgent_percentage": (
                (urgent_poles / total_poles * 100) if total_poles > 0 else 0
            ),
        }
        ...
        priority_costs = (
            schedule_df.groupby("maintenance_priority")["estimated_cost"]
            .sum()
            .to_dict()
        )

        report["by_priority"] = {"counts": priority_counts, "costs": priority_costs}

        # Budget analysis
        if budget_limit:
            budget_analysis = self._analyze_budget_constraints(
                schedule_df, budget_limit
            )
    ...
    def _analyze_budget_constraints(
        self, schedule_df: pd.DataFrame, budget_limit: float
    ) -> Dict:
    ...
    def _analyze_maintenance_timeline(self, schedule_df: pd.DataFrame) -> Dict:
        ...
        timeline_groups = (
            schedule_df.groupby("maintenance_window")
            .agg(
                {
                    "pole_id": "count",
                    "estimated_cost": "sum",
