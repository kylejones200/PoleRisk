...
from polerisk.ml.models import (
    SoilMoisturePredictor,
    AnomalyDetector,
    TimeSeriesForecaster,
)
from polerisk.ml.features import FeatureEngineer, select_features
from polerisk.analysis.statistics import (
    calculate_rmse,
    calculate_correlation,
    calculate_bias,
)
from polerisk.visualization.plots import create_dashboard
...
def get_data(filename: str):
    ...
    try:
        ...
        return jsonify(
            {"preview": data_preview, "shape": df.shape, "columns": list(df.columns)}
        )
...
def train_model():
    ...
    try:
        ...
        if "in_situ" not in df.columns:
            return (
                jsonify({"error": "No in_situ column found for training target"}),
                400,
            )
        ...
        if len(feature_cols) == 0:
            return (
                jsonify(
                    {
                        "error": "No features selected. Try lowering correlation threshold."
                    }
                ),
                400,
            )
        ...
        # Save model
        model_filename = (
            f"model_{model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.joblib"
        )
...
def make_predictions():
    ...
    try:
        ...
        if predictor.feature_names:
            feature_cols = [
                col for col in predictor.feature_names if col in features.columns
            ]
        else:
            feature_cols = [
                col for col in features.columns if col not in ["in_situ", "date"]
            ]
        ...
        for idx, pred in zip(X.index, predictions):
            ...
            if "in_situ" in features.columns and not pd.isna(
                features.loc[idx, "in_situ"]
            ):
        ...
        if "in_situ" in features.columns:
            ...
            if len(actual_values) > 0:
                metrics = {
                    ...
                    "correlation": float(
                        calculate_correlation(actual_values, pred_values)
                    ),
        ...
        return jsonify(
            {"predictions": results, "metrics": metrics, "model_used": model_filename}
        )
...
def detect_anomalies():
    ...
    try:
        ...
        # Prepare results
        results = []
        for idx, (is_anomaly, score) in zip(
            X.index, zip(anomaly_labels, anomaly_scores)
        ):
            result = {
...
def generate_forecast():
    ...
    try:
        ...
        forecaster = TimeSeriesForecaster(
            model_type=model_type, forecast_horizon=forecast_days
        )
        ...
        for date, forecast_value in zip(forecast_dates, forecasts):
            forecast_results.append(
                {"date": date, "forecast_value": float(forecast_value)}
            )
...
def load_sample_data():
    ...
    try:
        ...
        if not sample_files:
            return (
                jsonify({"error": "No sample data files found in Input directory"}),
                404,
            )
...
def create_synthetic_data_from_sample(sample_file: Path) -> pd.DataFrame:
    ...
    # Create DataFrame
    df = pd.DataFrame(
        {"date": dates, "in_situ": in_situ_values, "satellite": satellite_values}
    )
    ...
    logger.info(
        f"Satellite range: {df['satellite'].min():.3f} - {df['satellite'].max():.3f}"
    )
