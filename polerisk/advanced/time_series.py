...
class TimeSeriesAnalyzer:
    ...
    def _basic_stationarity_test(self, series: pd.Series) -> Dict:
        ...
        return {
            ...
            "conclusion": (
                "likely_stationary" if p_value > 0.05 else "likely_non_stationary"
            ),
        }

    def seasonal_decompose(
        self, series: pd.Series, model: str = "additive", period: Optional[int] = None
    ...
    def detect_changepoints(self, series: pd.Series, min_size: int = 30) -> List[Dict]:
        """
            ...
            if len(before) > 5 and len(after) > 5:
                ...
                if p_value < 0.01:  # Significant change
                    changepoint = {
                        "index": i,
                        "date": (
                            series_clean.index[i]
                            if hasattr(series_clean.index, "to_pydatetime")
                            else i
                        ),
                        "t_statistic": t_stat,
                        "p_value": p_value,
...
class TrendAnalyzer:
    ...
    def analyze_trend(self, series: pd.Series, method: str = "linear") -> TrendResult:
        """
        ...
        if len(series_clean) < 3:
            return TrendResult(
                0, 0, 0, 1, 0, "insufficient_data", "not_significant", "none"
            )
