...
class ClimatologyAnalyzer:
    ...
    def compute_climatology(
        self, data: pd.Series, reference_period: Optional[Tuple[str, str]] = None
    ) -> ClimatologyResult:
        """
    ...
    def detect_extreme_events(
        ...
        if threshold_type == "percentile":
            ...
            wet_threshold = (
                data_clean.max() - threshold_value
            ...
            try:
                ...
                seasonal_trends[season] = {
                    ...
                    "direction": (
                        "increasing"
                        if slope > 0
                        else "decreasing" if slope < 0 else "no_trend"
                    ),
...
class SeasonalAnalyzer:
    ...
    def analyze_seasonal_cycle(self, data: pd.Series) -> Dict:
        ...
        seasonal_var = (
            data_clean.groupby(data_clean.index.month).transform("mean").var()
        )
    ...
    def compare_years(
        self, data: pd.Series, reference_years: Optional[List[int]] = None
    ) -> Dict:
        """
        ...
        yearly_monthly = data_clean.groupby(
            [data_clean.index.year, data_clean.index.month]
        ).mean()
        ...
        if len(year_comparison) > 1:
            ...
            annual_amplitudes = [
                stats["seasonal_amplitude"] for stats in year_comparison.values()
            ]
            ...
            inter_annual_stats = {
                ...
                "cv_interannual": (
                    float(np.std(annual_means) / np.mean(annual_means))
                    if np.mean(annual_means) != 0
                    else np.inf
                ),
            }
        else:
    ...
    def phenology_analysis(
        self, data: pd.Series, growth_threshold: Optional[float] = None
    ) -> Dict:
        """
        ...
        for year in years:
            ...
            yearly_phenology[int(year)] = {
                ...
                "growing_season_length": (
                    (growing_season_end - growing_season_start).days
                    if growing_season_start and growing_season_end
                    else None
                ),
        ...
        if yearly_phenology:
            ...
            phenology_metrics = {
                "average_growing_season_length": (
                    float(np.mean(growing_lengths)) if growing_lengths else None
                ),
                "std_growing_season_length": (
                    float(np.std(growing_lengths)) if len(growing_lengths) > 1 else None
                ),
                "average_peak_day_of_year": (
                    float(np.mean(peak_doys)) if peak_doys else None
                ),
                "std_peak_day_of_year": (
                    float(np.std(peak_doys)) if len(peak_doys) > 1 else None
                ),
