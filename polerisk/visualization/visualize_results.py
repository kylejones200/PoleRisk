...
def plot_time_series(df, output_dir="Analysis"):
    ...
    # Formatting
    plt.title(
        "Time Series of Soil Moisture Measurements\nIn-situ vs. AMSR2 LPRM Satellite Data",
        fontsize=14,
        pad=20,
    )
...
def plot_vegetation_terrain_analysis(df, output_dir="Analysis"):
    ...
    if len(df) > 0:
        df["vegetation_type"] = np.random.choice(
            ["Forest", "Grassland", "Cropland", "Shrubland"], size=len(df)
        )
    ...
    if (
        "vegetation_type" in df.columns
        and "in_situ" in df.columns
        and "satellite" in df.columns
    ):
...
def plot_site_map(df, output_dir="Analysis"):
    ...
    try:
        ...
        ax.add_feature(
            cfeature.BORDERS, linestyle="-", linewidth=0.5, edgecolor="#888888"
        )
        ...
        logger.debug(
            "Cartopy is required for map generation. Install with: pip install cartopy"
        )
        return None
...
def create_dashboard(df, output_dir="Analysis"):
    ...
    ubrmse = np.sqrt(
        np.mean((diff - np.mean(diff)) ** 2)
    )  # Remove mean bias before RMSE
