...
class InteractiveMapGenerator:
    ...
    def create_site_map(
        self, sites_data: Union[pd.DataFrame, Dict], output_path: Optional[str] = None
    ) -> str:
        """
        ...
        center_lat = (
            sites_data["lat"].mean()
            if "lat" in sites_data.columns
            else self.default_center[0]
        )
        center_lon = (
            sites_data["lon"].mean()
            if "lon" in sites_data.columns
            else self.default_center[1]
        )
        ...
        for idx, row in sites_data.iterrows():
            if "lat" in row and "lon" in row:
                ...
                if "soil_moisture" in row and pd.notna(row["soil_moisture"]):
                    popup_content += (
                        f"<p><b>Soil Moisture:</b> {row['soil_moisture']:.3f} m³/m³</p>"
                    )
                ...
                if "elevation" in row and pd.notna(row["elevation"]):
                    popup_content += (
                        f"<p><b>Elevation:</b> {row['elevation']:.0f} m</p>"
                    )
    ...
    def create_heatmap(
        self, grid_data: pd.DataFrame, output_path: Optional[str] = None
    ) -> str:
        """
    ...
    def create_time_animation(
        self, time_series_data: pd.DataFrame, output_path: Optional[str] = None
    ) -> str:
        """
                    ...
                    feature = {
                        ...
                        "properties": {
                            ...
                            "style": {
                                ...
                                "fillColor": self._get_color_for_value(
                                    row["soil_moisture"]
                                ),
                                "fillOpacity": 0.7,
                                "radius": 8,
    ...
    def create_3d_surface(
        ...
        try:
            ...
            raise ImportError(
                "matplotlib 3D plotting is required for 3D visualizations"
            )
...
class GeospatialAnalyzer:
    ...
    def spatial_interpolation(
        self, known_points: pd.DataFrame, grid_resolution: float = 0.1
    ) -> pd.DataFrame:
        """
        ...
        interpolated = griddata(
            points, values, (lon_mesh, lat_mesh), method="linear", fill_value=np.nan
        )
        ...
        for i, lat in enumerate(lat_grid):
            for j, lon in enumerate(lon_grid):
                if not np.isnan(interpolated[i, j]):
                    result_data.append(
                        {"lat": lat, "lon": lon, "soil_moisture": interpolated[i, j]}
                    )
    ...
    def calculate_spatial_statistics(self, data: pd.DataFrame) -> Dict:
        ...
        lon_range_km = (
            stats["spatial_extent"]["lon_range"] * 111 * np.cos(np.radians(avg_lat))
        )
        ...
        # Site density
        stats["site_density_per_1000km2"] = len(data) / (
            stats["approximate_area_km2"] / 1000
        )

        return stats


def create_comprehensive_map_dashboard(
    data: pd.DataFrame, output_dir: str = "geospatial_output"
) -> List[str]:
    """
    ...
    try:
        ...
        logger.info(
            f"Generated {len(output_files)} geospatial visualizations in {output_dir}"
        )
