...
class PoleHealthVisualizer:
    ...
    def create_health_overview_dashboard(
        self,
        ...
        plt.tight_layout()
        plt.suptitle(
            "Utility Pole Health Assessment Dashboard",
            fontsize=20,
            fontweight="bold",
            y=0.98,
        )
    ...
    def _plot_risk_heatmap(self, df: pd.DataFrame, ax):
        ...
        if available_risks:
            ...
            ax.set_xticklabels(
                [
                    col.replace("_risk", "").replace("_", " ").title()
    ...
    def _plot_age_health_scatter(self, df: pd.DataFrame, ax):
        ...
        if "age_years" in df.columns and "overall_health_score" in df.columns:
            ...
            if len(df.dropna(subset=["age_years", "overall_health_score"])) > 1:
                z = np.polyfit(
                    df["age_years"].dropna(), df["overall_health_score"].dropna(), 1
                )
                p = np.poly1d(z)
    ...
    def _plot_pole_type_analysis(self, df: pd.DataFrame, ax):
        ...
        if "pole_type" in df.columns and "overall_health_score" in df.columns:
            pole_health = (
                df.groupby("pole_type")["overall_health_score"]
                .agg(["mean", "std"])
                .reset_index()
            )

            bars = ax.bar(
                pole_health["pole_type"],
                pole_health["mean"],
                yerr=pole_health["std"],
                capsize=5,
                alpha=0.7,
            )
    ...
    def _plot_cost_analysis(self, df: pd.DataFrame, ax):
        ...
        if "estimated_cost" in df.columns and "maintenance_priority" in df.columns:
            cost_by_priority = df.groupby("maintenance_priority")[
                "estimated_cost"
            ].sum()
        ...
        if "days_until_action" in df.columns and "maintenance_priority" in df.columns:
            ...
            for priority in priority_order:
                if priority in df["maintenance_priority"].values:
                    days = df[df["maintenance_priority"] == priority][
                        "days_until_action"
                    ]
    ...
    def create_interactive_map(
        ...
        if (
            "latitude" not in assessment_df.columns
            or "longitude" not in assessment_df.columns
        ):
    ...
    def create_trend_analysis(
        self, soil_history: pd.DataFrame, output_dir: str = "Analysis"
    ) -> str:
        ...
        if "moisture_content" in soil_history.columns:
            for pole_id in soil_history["pole_id"].unique():
                ...
                axes[0, 0].plot(
                    pole_data["sample_date"],
                    pole_data["moisture_content"],
                    marker="o",
                    label=pole_id,
                    alpha=0.7,
                )
        ...
        if "electrical_conductivity" in soil_history.columns:
            for pole_id in soil_history["pole_id"].unique():
                ...
                if not pole_data["electrical_conductivity"].isna().all():
                    axes[1, 0].plot(
                        pole_data["sample_date"],
                        pole_data["electrical_conductivity"],
                        marker="^",
                        label=pole_id,
                        alpha=0.7,
                    )
    ...
    def generate_visual_report(
        ...
        dashboard_file = self.create_health_overview_dashboard(
            assessment_df, schedule_df, output_dir
        )
