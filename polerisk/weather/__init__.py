...
class OpenWeatherMapProvider(WeatherDataProvider):
    ...
    def get_forecast(
        ...
        try:
            ...
            for date_key, day_data in list(daily_data.items())[:days]:
                ...
                winds = [
                    item["wind"]["speed"] * 2.237 for item in day_data
                ]  # m/s to mph
                precip_prob = max(
                    [item["pop"] * 100 for item in day_data]
                ...
                forecast = WeatherForecast(
                    ...
                    wind_gust_mph=max(
                        [item["wind"].get("gust", 0) * 2.237 for item in day_data]
                    ),
                    weather_description=midday_item["weather"][0]["description"],
                    humidity_percent=np.mean(
                        [item["main"]["humidity"] for item in day_data]
                    ),
                )
            ...
            if "alerts" in data:
                for alert_data in data["alerts"]:
                    alert = WeatherAlert(
                        ...
                        urgency=(
                            "immediate"
                            if alert_data["start"] <= datetime.now().timestamp()
                            else "expected"
                        ),
                        ...
                        pole_risk_multiplier=self._calculate_pole_risk_multiplier(
                            alert_data["event"]
                        ),
                    )
                    alerts.append(alert)
    ...
    def _get_mock_current_conditions(
        self, latitude: float, longitude: float
    ) -> WeatherConditions:
        ...
        return WeatherConditions(
            ...
            precipitation_mm=(
                np.random.uniform(0, 5) if np.random.random() > 0.7 else None
            ),
...
class WeatherRiskAssessment:
    ...
    def assess_forecast_risk(
        ...
        return {
            ...
            "risk_trend": (
                "increasing" if len(risks) > 3 and risks[-1] > risks[0] else "stable"
            ),
        }

    def get_weather_enhanced_pole_risk(
        self, latitude: float, longitude: float
    ) -> Dict[str, float]:
