...
class SMAPProcessor:
    ...
    def read_smap_file(
        self, filepath: Union[str, Path], product_type: str = "SPL3SMP"
    ) -> Dict:
        """
        Read SMAP HDF5 file.
    ...
    def extract_point_data(
        ...
        for qf_name in ["quality_flag", "quality_flag_am", "quality_flag_pm"]:
            if qf_name in smap_data:
                ...
                point_data[f"{qf_name}_description"] = self.quality_flags.get(
                    qf_value, "unknown"
                )
        ...
        for filepath in file_list:
            try:
                ...
                row_data = {
                    ...
                    "distance_km": point_data["distance"]
    ...
    def validate_data_quality(self, point_data: Dict) -> Dict:
        ...
        if sm_value is not None:
            if np.isnan(sm_value):
                ...
                quality_assessment["issues"].append(
                    f"Soil moisture out of range: {sm_value}"
                )
        ...
        for qf_name in ["quality_flag", "quality_flag_am", "quality_flag_pm"]:
            if qf_name in point_data:
                ...
                if qf_value > 2:
                    quality_assessment["issues"].append(
                        f"{qf_name} indicates poor quality: {qf_value}"
                    )
        ...
        if distance > 0.25:  # > 0.25 degrees
            quality_assessment["issues"].append(
                f"Data point far from target: {distance:.3f}°"
            )

        # Overall quality assessment
        if not quality_assessment["issues"]:
            quality_assessment["overall_quality"] = "good"
        elif len(quality_assessment["issues"]) <= 2 and all(
            qf <= 1 for qf in quality_scores
        ):
        ...
        if distance > 0.1:
            quality_assessment["recommendations"].append(
                "Consider using higher resolution product"
            )
