...
class DataIngestionPipeline:
    ...
    def start_watching(self) -> bool:
        ...
        try:
            ...
            self.observer.schedule(
                event_handler, self.config.source_path, recursive=True
            )
        ...
        try:
            ...
            if self.config.processing_function:
                try:
                    ...
                    logger.error(
                        f"Processing function failed for {file_path.name}: {e}"
                    )
    ...
    def _wait_for_file_stability(
        self, file_path: Path, stability_time: int = 2
    ) -> bool:
    ...
    def _move_to_error_dir(self, file_path: Path, error_msg: str):
        ...
        try:
            ...
            error_log_path = (
                Path(self.config.error_files_dir) / f"{file_path.name}.error.txt"
            )
...
class ScheduledIngestion:
    ...
    def add_daily_job(
        self, job_name: str, pipeline: DataIngestionPipeline, time_str: str = "02:00"
    ):
    ...
    def add_custom_job(
        self, job_name: str, pipeline: DataIngestionPipeline, schedule_func: Callable
    ):
