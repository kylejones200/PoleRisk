...
class SoilSample(Base):
    ...
    id = Column(Integer, primary_key=True)
    pole_id = Column(
        String(50), ForeignKey("poles.pole_id"), nullable=False, index=True
    )
...
class StructuralInspection(Base):
    ...
    )  # visual, detailed, resistograph, etc.
...
class WorkOrder(Base):
    ...
    work_type = Column(
        String(30), nullable=False
    )  # inspection, maintenance, replacement, emergency
...
class PoleDataAccess:
    ...
    def get_latest_health_assessment(self, pole_id: str) -> HealthAssessment:
        ...
        try:
            return (
                session.query(HealthAssessment)
