...
def find_nearest_valid_pixel_numba(
    data: np.ndarray, target_row: int, target_col: int, search_radius: int = 3
...
def calculate_distance_matrix_numba(
    target_lat: float, target_lon: float, lat_grid: np.ndarray, lon_grid: np.ndarray
) -> np.ndarray:
    """
...
def find_nearest_grid_point_numba(
    ...
    if lat_idx > 0 and (
        lat_idx == len(lat_grid)
        or abs(lat_grid[lat_idx - 1] - target_lat) < abs(lat_grid[lat_idx] - target_lat)
    ):
        lat_idx -= 1
    ...
    if lon_idx > 0 and (
        lon_idx == len(lon_grid)
        or abs(lon_grid[lon_idx - 1] - target_lon) < abs(lon_grid[lon_idx] - target_lon)
    ):
        lon_idx -= 1
...
def batch_statistical_analysis_numba(
    data_matrix: np.ndarray, reference_values: np.ndarray
