...
class LPRMDataLoader:
    ...
    def find_nearest_valid_pixel(
        self, data: np.ndarray, row: int, col: int, search_radius: int = 3
    ) -> Optional[np.ndarray]:
...
def extract_pixel_data(
    ...
    try:
        ...
        raise ValueError(
            f"Could not find grid cell for location ({target_lat}, {target_lon}): {str(e)}"
        )
    ...
    if (
        lat_idx < 0
        or lat_idx >= data.shape[1]
        or lon_idx < 0
        or lon_idx >= data.shape[2]
    ):
        raise ValueError(
            f"Grid indices ({lat_idx}, {lon_idx}) out of bounds for data shape {data.shape}"
        )
