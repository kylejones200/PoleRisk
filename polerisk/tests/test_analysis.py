...
from polerisk.analysis.statistics import (  # noqa: E402
    StatisticalAnalyzer,
    calculate_bias,
...
class TestStatisticalAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.predicted_values = self.true_values + np.random.normal(
            0, 0.05, self.n_samples
        )

        # Create DataFrame
        self.test_data = pd.DataFrame(
            {"observed": self.true_values, "predicted": self.predicted_values}
        )
    ...
    def test_analysis_with_identical_data(self):
        ...
        identical_data = pd.DataFrame(
            {"observed": self.true_values, "predicted": self.true_values}
        )
    ...
    def test_analysis_with_constant_data(self):
        ...
        constant_data = pd.DataFrame(
            {"observed": np.full(50, 0.3), "predicted": np.full(50, 0.3)}
        )
    ...
    def test_seasonal_analysis(self):
        ...
        seasonal_data = pd.DataFrame(
            {
                "date": dates,
                "observed": 0.3
                + 0.1 * seasonal_pattern
                + np.random.normal(0, 0.02, len(dates)),
                "predicted": 0.3
                + 0.08 * seasonal_pattern
        ...
        seasonal_results = self.analyzer.seasonal_analysis(
            seasonal_data, "observed", "predicted", "month"
        )
    ...
    def test_outlier_detection(self):
        ...
        data_with_outliers.loc[outlier_indices, "predicted"] = (
            2.0  # Unrealistic soil moisture
        )

        outliers = self.analyzer.detect_outliers(
            data_with_outliers, "observed", "predicted"
        )
    ...
    def test_trend_analysis(self):
        ...
        trend_results = self.analyzer.trend_analysis(
            trend_data, "observed", "predicted"
        )
