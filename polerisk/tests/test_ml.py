...
from polerisk.ml.features import (
    FeatureEngineer,
...
def sample_data():
    ...
    data = {
        ...
        "precipitation": np.random.exponential(
            2, n_days
        ),  # Exponential distribution for precip
    }
...
class TestAnomalyDetector:
    ...
    def test_statistical_detection(self, sample_data):
        ...
        assert len(anomaly_labels) == len(X)
        assert all(
            score >= 0 for score in anomaly_scores
...
class TestFeatureEngineer:
    ...
    def test_fit_transform(self, sample_data):
        ...
        assert len(features) <= len(
            sample_data
        )  # Some rows might be dropped due to rolling ops
...
class TestFeatureFunctions:
    ...
    def test_feature_selection(self, sample_data):
        ...
        selected = select_features(
            features, target_col="in_situ", correlation_threshold=0.01, max_features=10
        )
...
class TestIntegration:
    ...
    def test_end_to_end_pipeline(self, sample_data):
        ...
        feature_cols = select_features(
            features, target_col="in_situ", correlation_threshold=0.01, max_features=15
        )
