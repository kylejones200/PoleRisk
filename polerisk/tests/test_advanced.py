...
from polerisk.advanced.time_series import TimeSeriesAnalyzer, TrendAnalyzer
from polerisk.advanced.climatology import ClimatologyAnalyzer, SeasonalAnalyzer
...
class TestTimeSeriesAnalyzer:
    ...
    def test_filter_application(self, sample_time_series):
        ...
        filtered = analyzer.apply_filter(
            sample_time_series, filter_type="moving_average"
        )
        ...
        try:
            filtered_butter = analyzer.apply_filter(
                sample_time_series, filter_type="butterworth", cutoff_freq=0.1
            )
...
class TestClimatologyAnalyzer:
    ...
    def test_compute_anomalies(self, sample_time_series):
        ...
        abs_anomalies = analyzer.compute_anomalies(
            sample_time_series, anomaly_type="absolute"
        )
        rel_anomalies = analyzer.compute_anomalies(
            sample_time_series, anomaly_type="relative"
        )
        std_anomalies = analyzer.compute_anomalies(
            sample_time_series, anomaly_type="standardized"
        )
    ...
    def test_detect_extreme_events(self, sample_time_series):
        ...
        extremes = analyzer.detect_extreme_events(
            sample_time_series, threshold_type="percentile", threshold_value=5.0
        )
...
class TestSeasonalAnalyzer:
    ...
    def test_custom_growth_threshold(self, sample_time_series):
        ...
        phenology = analyzer.phenology_analysis(
            sample_time_series, growth_threshold=custom_threshold
        )
...
class TestIntegration:
    ...
    def test_complete_analysis_workflow(self, sample_time_series):
        ...
        logger.debug(
            f"Seasonality strength: {seasonal_cycle['seasonality_strength']:.3f}"
        )
