...
def assess_pole_fleet(poles: list, soil_samples: list, output_dir: str = "Output"):
    ...
    # Group soil samples by pole using defaultdict for cleaner code
    from collections import defaultdict

    samples_by_pole = defaultdict(list)
    for sample in soil_samples:
        samples_by_pole[sample.pole_id].append(sample)

    # Constants for parallel processing threshold
    PARALLEL_THRESHOLD = 50  # Use parallel processing for 50+ poles

    # Assess each pole - use parallel processing for large batches
    if len(poles) >= PARALLEL_THRESHOLD:
        from concurrent.futures import ProcessPoolExecutor, as_completed
        from functools import partial

        logger.info(f"Processing {len(poles)} poles in parallel...")

        def assess_single_pole(pole, pole_samples, assessor, risk_scorer):
            """Assess a single pole - designed for parallel execution."""
            if not pole_samples:
                return None

            health_metrics = assessor.assess_pole_health(pole, pole_samples)
            risk_scores = risk_scorer.calculate_comprehensive_risk_score(
                pole, health_metrics
            )
            return (pole, health_metrics, risk_scores)
        ...
        with ProcessPoolExecutor(max_workers=None) as executor:
            ...
            future_to_pole = {
                executor.submit(
                    assess_single_pole,
                    pole,
                    samples_by_pole.get(pole.pole_id, []),
                    assessor,
                    risk_scorer,
                ): pole
                for pole in poles
            ...
            for future in as_completed(future_to_pole):
                pole = future_to_pole[future]
                try:
                    result = future.result()
                    if result is not None:
                        assessment_results.append(result)
                except Exception as e:
                    logger.warning(f"Error assessing pole {pole.pole_id}: {e}")

                    # Fallback to sequential if no samples
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to assess pole {pole.pole_id}: {fallback_error}"
                        )
    else:
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(
            f"Total Estimated Maintenance Cost: ${summary['total_estimated_cost']:,.2f}\n"
        )
  ...
  python main.py --poles Input/poles.csv --soil Input/soil_data.csv --output Results/
    ...
    try:
        if args.create_sample_data:
            ...
            logger.info(
                f"Run: python {sys.argv[0]} --poles {poles_file} --soil {soil_file}"
            )
            return
        ...
        # Show immediate attention poles (vectorized filtering)
        urgent_poles = schedule[schedule["requires_immediate_attention"]]
        if len(urgent_poles) > 0:
            ...
            for pole in urgent_poles.itertuples(index=False):
