...
from soilmoisture.pole_health.enhanced_models import (
    StructuralInspection, InspectionType, PoleCondition, EnhancedPoleInfo
)
...
def create_sample_structural_inspections():
    ...
    for pole_id in pole_ids:
        ...
        inspection = StructuralInspection(
            ...
            damage_description="Minor surface weathering" if random.random() > 0.5 else None,
            
            # Material-specific data based on pole type
            wood_decay_depth=random.uniform(0.5, 2.5) if pole_id == 'P001' else None,
            wood_circumferential_loss=random.uniform(5, 35) if pole_id == 'P001' else None,
            concrete_cracking=random.choice([True, False]) if pole_id == 'P002' else None,
            concrete_spalling=random.choice([True, False]) if pole_id == 'P002' else None,
            steel_corrosion_level=random.randint(1, 4) if pole_id == 'P003' else None,
            coating_condition=random.choice(['excellent', 'good', 'fair', 'poor']) if pole_id == 'P003' else None,
            ...
            # Documentation
            photo_paths=[f"photos/{pole_id}_inspection_1.jpg", f"photos/{pole_id}_inspection_2.jpg"],
            notes=f"Routine inspection of pole {pole_id}. Overall condition appears stable.",
            confidence_level=random.uniform(0.8, 1.0),
            recommended_action="Continue monitoring" if random.random() > 0.3 else "Schedule follow-up inspection",
            next_inspection_date=datetime.now() + timedelta(days=random.randint(180, 720))
        )
...
def test_structural_analysis():
    ...
    for inspection in inspections:
        ...
        logger.debug(f"Inspection Date: {inspection.inspection_date.strftime('%Y-%m-%d')}")
        ...
        condition_score = analyzer.calculate_overall_condition_score(inspection, pole_type)
        logger.debug(f"Calculated Condition Score: {condition_score:.1f}/100")
        
        # Get inspection frequency recommendation
        pole_age = 15 if inspection.pole_id == 'P001' else (20 if inspection.pole_id == 'P002' else 10)
        frequency = analyzer.determine_inspection_frequency(condition_score, pole_age, pole_type)
        logger.debug(f"Recommended Inspection Frequency: {frequency} months")
        
        # Get maintenance recommendations
        recommendations = analyzer.generate_recommendations(inspection, condition_score, pole_type)
