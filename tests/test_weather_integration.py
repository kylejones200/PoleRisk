...
def test_weather_integration():
    ...
    for lat, lon, pole_id in test_locations:
        ...
        if conditions:
            logger.debug(f"  Current: {conditions.temperature_c:.1f}°C, {conditions.humidity_percent:.0f}% humidity")
            logger.debug(f"  Wind: {conditions.wind_speed_mph:.1f} mph @ {conditions.wind_direction_deg:.0f}°")
        ...
        if forecasts:
            ...
            for i, forecast in enumerate(forecasts[:3]):
                logger.debug(f"    Day {i+1}: {forecast.temp_min_c:.1f}°C - {forecast.temp_max_c:.1f}°C, "
        ...
        if alerts:
            ...
            for alert in alerts:
                logger.debug(f"    {alert.title}: {alert.severity} ({alert.alert_type})")
        else:
        ...
        logger.debug(f"  Current weather risk: {current_risks['overall_weather_risk']:.3f}")
        logger.debug(f"  Forecast risk: {forecast_risks['forecast_risk']:.3f}")
        logger.debug(f"  Combined weather risk: {combined_risks['combined_weather_risk']:.3f}")
...
def test_weather_enhanced_assessment():
    ...
    try:
        ...
        for _, row in assessment_df.iterrows():
            ...
            weather_adjustment = weather_risks['combined_weather_risk'] * 10  # Max 10 point reduction
            original_score = enhanced_record['overall_health_score']
            enhanced_record['weather_adjusted_health_score'] = max(0, original_score - weather_adjustment)
            
            enhanced_data.append(enhanced_record)
            
            logger.info(f"  {pole_id}: Original score {original_score:.1f} -> "
