...
class FailureModeRisk:
    ...
    time_to_failure_confidence: Optional[Tuple[float, float]] = None  # (lower, upper) bounds
...
class FailureModeModel:
    ...
    def assess_failure_modes(self, pole: PoleInfo, 
                            health_metrics: PoleHealthMetrics,
                            environmental_data: Optional[Dict] = None) -> FailureModeAnalysis:
        """
        ...
        for mode in FailureMode:
            risk = self._assess_single_mode(mode, pole, health_metrics, features, environmental_data)
    ...
    def _extract_features(self, pole: PoleInfo, 
                         health_metrics: PoleHealthMetrics,
        ...
        features['wood_type'] = self._encode_wood_type(getattr(pole, 'material', 'unknown'))
        ...
        features['groundline_condition'] = 1.0 - soil_stability  # Lower score = worse condition
    ...
    def _assess_single_mode(self, mode: FailureMode, pole: PoleInfo,
                           health_metrics: PoleHealthMetrics,
                           features: Dict[str, float],
                           environmental_data: Dict) -> FailureModeRisk:
        ...
        recommended_actions = self._get_recommended_actions(mode, risk_score, probability)
        
        # Estimate time to failure if high risk
        time_to_failure = None
        time_confidence = None
        if probability > 0.01:  # 1% annual probability
            time_to_failure, time_confidence = self._estimate_time_to_failure(probability)
    ...
    def _calculate_confidence(self, mode: FailureMode, features: Dict, predictors: Dict) -> float:
        ...
        for factor, weight in predictors.items():
            total_weight += weight
            if features.get(factor) is None or features.get(factor) == 0.5:  # Default value
    ...
    def _get_inspection_methods(self, mode: FailureMode, risk_score: float) -> List[str]:
        ...
        base_methods = {
            ...
            FailureMode.FOUNDATION_FAILURE: ['visual', 'foundation_inspection', 'soil_test'],
        }
    ...
    def _get_recommended_actions(self, mode: FailureMode, risk_score: float, 
        ...
        mode_specific_actions = {
            ...
            FailureMode.HARDWARE_FAILURE: ['hardware_replacement', 'corrosion_protection'],
            FailureMode.ICE_LOAD: ['load_management', 'de_icing_equipment'],
            FailureMode.FOUNDATION_FAILURE: ['foundation_stabilization', 'erosion_control'],
        }
        
        actions.extend(mode_specific_actions.get(mode, []))
        return actions
    
    def _estimate_time_to_failure(self, annual_probability: float) -> Tuple[float, Tuple[float, float]]:
        """
        ...
        lower = -np.log(0.95) / annual_probability if annual_probability > 0 else expected_ttf * 0.5
        upper = -np.log(0.05) / annual_probability if annual_probability > 0 else expected_ttf * 2.0
